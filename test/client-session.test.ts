...
describe('native session launch', () => {
  ...
  it('launches an actual child with the chosen cwd/identity and propagates its exit', async () => {
    ...
    writeFileSync(script, 'console.log(JSON.stringify({cwd:process.cwd(),agent:process.env.HEDDLE_AGENT,client:process.env.HEDDLE_CLIENT,push:process.env.HEDDLE_COMMS_PUSH}));process.exitCode=7;');
    ...
    expect(JSON.parse(result.stdout)).toEqual({ cwd: dir, agent: 'codex-test', client: 'opencode', push: '0' });
  });
});
