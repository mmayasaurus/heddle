...
describe('ledger adversarial reviews', () => {
  ...
  it('upserts reviewer identity without erasing the existing mandate or scored outcome', () => {
    const db = ledger(); const id = start(db);
  ...
  it('aggregates scored review pairs and orders the largest pair first', () => {
    const db = ledger();
    const a = start(db, 1); review(db, a, 'claude', 'cursor'); db.recordReviewOutcome(a, { findingsTotal: 5, findingsAccepted: 3 });
    const b = start(db, 2); review(db, b, 'claude', 'cursor'); db.recordReviewOutcome(b, { findingsTotal: 4, findingsAccepted: 4 });
    const c = start(db, 3); review(db, c, 'claude', 'gemini');
    const d = start(db, 4); review(db, d, 'codex', 'claude'); db.recordReviewOutcome(d, { findingsTotal: 2, findingsAccepted: 0 }); db.setReviewMandate(d, false);
