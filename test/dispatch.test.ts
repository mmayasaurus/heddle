...
describe('dispatch — class + explicit route, and in-session refusal', () => {
  ...
  it('replaces class default skills with explicit skills while retaining worker-role', async () => {
    ...
    expect(outcome.skills).toEqual(['worker-role', 'worker-hygiene', 'code-discovery', 'family-codex']);
  });
  ...
  it('refuses headless Claude opus and fable adversarial reviews before an adapter is dispatched', async () => {
    for (const model of ['opus', 'fable']) {
      const fake = fakeAdapter();
      const outcome = await dispatch(
        { taskClass: 'adversarial-review', provider: 'claude', model, authorProvider: 'codex', prompt: 'review', cwd: tempDir() },
        tempLedger(), () => fake.adapter,
      );
