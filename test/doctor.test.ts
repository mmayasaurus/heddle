...
describe('runDoctor', () => {
  ...
  test('regression: secrets-file keys are reported only as present and never leak into text or JSON', async () => {
    ...
    writeFileSync(secrets, 'GROQ_API_KEY="not-a-real-key"\n');
  ...
  test('regression: capped real catalogs retain leading models and report trailing missing models', async () => {
...
case "$1" in
  ...
  models) echo 'cursor-good - Good'; yes filler | head -c 300000 ;;
esac
`,
    );
...
case "$1" in
  --version) echo 1.0 ;;
  status) echo '{"isAuthenticated":true}' ;;
  models) yes filler | head -c 300000; echo 'cursor-good - Good' ;;
esac
`,
    );
  ...
  test('regression: failed catalog streams cannot leak a token or identity into the report', async () => {
    const report = await runDoctor({ provider: 'cursor' }, fakeDeps(undefined, { execFile: async (cmd, args, opts) => {
      if (cmd === 'cursor-agent' && args[0] === 'models') return { stdout: '', stderr: 'token=not-a-real-key email=leak@example.com', exitCode: 1, timedOut: false };
      return fakeDeps().execFile!(cmd, args, opts);
    } }));
    const output = `${formatDoctorReport(report)}\n${JSON.stringify(report)}`;
    expect(output).not.toContain('not-a-real-key');
