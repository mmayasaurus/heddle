...
describe('Ledger — caps, lineage trigger, migration batch', () => {
  const { tempDir, trackLedger } = useTempResources('heddle-ledger-caps-test-');
  const cap = { max: 2, staleAfterMs: 3 * 60 * 60 * 1000 };
  function path(): string { return join(tempDir(), 'ledger.db'); }
  ...
  it('enforces immutable id and orchestrator lineage while allowing outcome updates', () => {
    const dbPath = path();
    const ledger = trackLedger(new Ledger(dbPath));
