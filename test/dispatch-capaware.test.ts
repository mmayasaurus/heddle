...
describe('dispatch cap-aware decisions', () => {
  ...
  it('includes Claude account advice in an in-session refusal but routes away before refusing an exhausted primary', async () => {
    ...
    const ledger = tempLedger(); const refused = await dispatch({ taskClass: 'implementation', prompt: 'x', cwd: tempDir(), identity: unbound, caps: { claude }, accounts }, ledger, () => fakeAdapter().adapter);
    expect(refused).toMatchObject({ refusal: { code: 'claude-in-session' }, account: 'acct2' }); expect(refused.refusal?.instruction).toContain('acct2 has the most 5h headroom'); expect(ledger.recent(1)[0].account).toBe('acct2');
