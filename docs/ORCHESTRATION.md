...
> Goal: turn Maya's existing lettered Claude sessions (Agents A–Q) into **orchestrators** that
> decompose their claimed Linear issues and dispatch sub-tasks to best-fit models — usable for
> real Spinventory work while the dashboard is built in parallel. Nothing here requires the GUI.
> Decisions locked 2026-08-03.

## What an orchestrator does (the loop we're enabling)

1. Claims SPI issue via existing `lin.sh` (unchanged — Commandment protocol intact).
...
5. Records every dispatch to the ledger; asks Maya via the needs-attention queue when blocked.
...
- `heddle ledger [--issue SPI-n]` — dispatch history; feeds dashboard + savings stats later.
...
Packs start small and Spinventory-specific: `sleek-style`, `social-style`, `db-supabase-dev`,
...
### 4. Chatroom (fleet comms) — PULL model, per Maya 2026-08-03
...
- **Subagent↔subagent and subagent↔host DMs** are first-class, not only room posts. Maya can DM
  any agent or subagent.
- Implementation: broker thread + a small TUI client → **works in plain iTerm tabs before any
  GUI exists**.

### 5. Ledger + hooks
- SQLite: dispatches (orchestrator, worker, provider, model, skills, task class, SPI issue, PR,
  usage, duration, outcome), chat messages, needs-attention items, session registry.
- Worker-scoped hook configs (never edits Maya's global CLI configs) append activity events —
  all four CLIs have JSON-on-stdin hook systems.

### 6. Docs / rules integration
- `.claude/rules/` stub in the Spinventory repo pointing at the orchestration protocol (load-on-
...
5. **Pilot on one real SPI issue** with an existing lettered agent; measure tokens/outcome vs an
   all-Claude baseline; tune the routing table from ledger data.
6. Roll out to the rest of the lettered fleet.
