...
HED-394's headless-first invariant makes the heddle repository's `main` the product source of
truth: every `[HEADLESS]` child lands there before it ships in a pack artifact—never from a side
branch or pack-only patch. HED-419's standalone generator must be run from a clean `main` checkout so
the shipped snapshot cannot diverge from the source of truth; mechanical enforcement—`heddle release
--standalone` rejects a dirty tree or a source that is not at main's HEAD—landed in HED-507.

The standalone repository is generated by that release script, never hand-maintained, so it cannot
structurally diverge. Its only permitted divergence surface is configuration: internal installs keep
their values in `~/.heddle` and `heddle.toml` (HED-407), while the pack carries defaults. Features
motivated internally must be carved as the generic provider/registry path—not an internal special
case—so they flow into the pack for free (for example, HED-422 through the HED-395 shape). The
shareable repository remains CLI-only: dashboard/UI code lives only in `heddle-dashboard`, and
“headless” means the universal Claude Code CLI version the operator runs, not anything operator- or project-specific.
