...
HED-394's headless-first invariant makes `main` in `mmayasaurus/heddle` the product source of
truth: every `[HEADLESS]` child lands there before it ships in a pack artifact—never from a side
branch or pack-only patch. HED-419's standalone generator is generated from a clean `main` HEAD;
`assertMainHeadSource()` rejects any other branch, dirty checkout, or ref outside that HEAD.
...
“headless” means the universal Claude Code CLI version Maya runs, not anything Spinventory-specific.
