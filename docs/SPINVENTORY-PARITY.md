...
| — (cross-cutting, surfaced by this audit) | Hook DISTRIBUTION: the SILENT file-existence guards are gone — every hook now runs behind a loud-fail-open guard (absent → stderr banner + skip, never a silent vanish), and the 3 location-independent hooks (agent-identity, delegation-nudge, remind-owned-prs) are vendored into `.claude/hooks/` (heddle repo; heddle-dashboard same-fix pending). The 2 location-coupled deep hooks (require-memtrace-first, require-pr-sweep) still resolve to the Spinventory canonical, behavior-neutral, pending HED-96 | **PARTIAL** | HED-107 (bridge — done) → HED-96 (canonical → ~/.heddle, deep-hook vendoring, deliberate enforce-flip). Caught by codex review on heddle#22 |
