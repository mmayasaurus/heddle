...
| — (cross-cutting, surfaced by this audit) | Hook DISTRIBUTION: heddle's entire hook layer resolves through one absolute path (`/Users/mayatobi/Developer/Spinventory-Rebuild-App/.claude/hooks/...`) behind silent file-existence guards — if that checkout moves/retires, identity, gates, and telemetry vanish invisibly | **GAP** | HED-107 (vendor / relocatable resolution / hard-fail bootstrap). Caught by codex review on heddle#22 |
...
| heddle-self-merge.md | heddle-native (written for HED) + config-text exception with same-breath overlap measurement | **BETTER** | Exercised on heddle#18 + dashboard#22 merges 2026-08-16 (merge bodies state the measurements) |
...
| Spinventory | heddle | Verdict | Notes & receipt |
|---|---|---|---|
| CONTEXT.md entry-point ritual ("Current State", where-things-live) | CLAUDE.md (rule loader) exists in both repos; the fleet decision log is `ROADMAP.md` **in the dashboard repo only**; no living current-state doc + no session-start read ritual in either repo | **GAP** | HED-100 |
| AGENTS.md Ten Commandments (single reference) | Individual rules cover the commandments — several with harder enforcement (rulesets, hard gates) — but no single entry-point | **PARITY** | Enforcement ≥ per rows above; single-doc consolidation folded into HED-100 |
...
| .session/state.md compaction protocol | Not deployed for heddle. Honest note: Spinventory's `.session/` is **empty** — protocol documented globally, artifacts unused | **GAP** | HED-100 (port the protocol only if it actually fires — hook-assisted — not as documentation-ware) |
