 ...
 * 98% 5h or 7d accounts. decideRoute's DISPATCH callers (claudeRouteDead, planDispatch, the runtime
 ...
 *
 * Floors come from lanes.yaml (`floors.claude`, ratified). This module is pure over (5h used%, 7d
 * used%, floors).
 */
export interface ClaudeFloors {
  /** Never SELECT an account whose lower 5h-or-7d HEADROOM is below this (headroom = 100 − used%). */
...
/**
 * Is this account FLOORED — too close to exhaustion to route or resume onto? True when either known
 * meter's headroom is at or below `neverBelowPct` (INCLUSIVE): min(known headrooms) ≤
 * never_below_pct → floored, so at pct=3, used ≥ 97% in either meter is floored. The field NAME
 * "never_below" reads exclusive, but the RATIFIED
 ...
 * have stopped the rollover from resuming onto a 98% 7d account.
 */
export function isFloored(used5hPct: number | null, used7dPct: number | null, floors: ClaudeFloors): boolean {
  const headrooms = [headroomPct(used5hPct), headroomPct(used7dPct)].filter((headroom): headroom is number => headroom !== null);
  return headrooms.length > 0 && Math.min(...headrooms) <= floors.neverBelowPct;
}

/** Which known meter has the lower headroom; ties intentionally prefer 5h. Both unknown → null. */
export function bindingMeter(used5hPct: number | null, used7dPct: number | null): '5h' | '7d' | null {
  const headroom5h = headroomPct(used5hPct);
  const headroom7d = headroomPct(used7dPct);
  if (headroom5h === null) return headroom7d === null ? null : '7d';
  if (headroom7d === null || headroom5h <= headroom7d) return '5h';
  return '7d';
}
