...
export async function runDoctor(
  ...
  const routingPath = deps.paths.routing ?? deps.env.HEDDLE_ROUTING ?? defaultRoutingPath();
  const lanesPath = deps.paths.lanes ?? deps.env.HEDDLE_LANES ?? defaultLanesPath();
  const accountsPath = deps.paths.accounts ?? deps.env.HEDDLE_ACCOUNTS ?? DEFAULT_ACCOUNTS_PATH;
  const lanes: { ok: true; value: LanesConfig } | { ok: false; error: string } = (() => {
    try {
  ...
  const missing = new Set<HarnessProvider>();
  ...
  for (const harness of harnesses) {
    if (!selected.has(harness.provider)) {
      continue;
    }
    ...
    definitions.push({
      ...
      provider: harness.provider,
      run: async () => {
        if (missing.has(harness.provider)) {
    ...
    definitions.push({
      ...
      run: async () => {
        ...
        const targets = targetModels(harness.provider, routingPath);
  ...
  definitions.push({
    id: 'config:lanes',
    kind: 'config',
    run: async () => (lanes.ok ? result('ok', 'lanes.yaml parses') : result('fail', lanes.error)),
  });
  definitions.push({
    id: 'config:projects',
    kind: 'config',
    ...
    definitions.push({
      ...
      run: async () => {
        ...
        const days = Math.floor(
          (deps.now().getTime() - new Date(config.lastVerified).getTime()) / 86_400_000,
        );
        const present = Boolean(readSecretsEnvValue(config.keyEnv, deps.paths.secrets));
        const verification =
          days > lanes.value.floors.menial_verify_days
            ? `last verified ${days} days ago (> ${lanes.value.floors.menial_verify_days})`
            : `last verified ${days} days ago`;

        return result(
          days > lanes.value.floors.menial_verify_days || !present ? 'warn' : 'ok',
