 ...
 * Only agent/child addresses bind from those sources. THE OPERATOR binds differently — by an
 * explicit, configuration-level credential that a model cannot set for itself:
 *   HEDDLE_COMMS_ROLE=operator + HEDDLE_COMMS_OPERATOR_TOKEN equal to the contents of
 *   ~/.heddle/operator.token (create it once with `heddle-comms --init-operator-token`, 0600).
 * The token lives in Maya's own session config (.mcp.json env), which is exactly the surface
 * only she edits — so "origin-verified" here means "the session was configured as hers", and her
 * posts get the `operator` tier, never wrapped untrusted. A worker (HEDDLE_WORKER=1) may not mint
 * children — depth 1.
 *
...
const OPERATOR_TOKEN_PATH = process.env.HEDDLE_OPERATOR_TOKEN_PATH || join(homedir(), '.heddle', 'operator.token');
...
log.ensureDefaultRooms(); // #fleet — everyone; per-lane rooms are created on demand by orchestrators/operator
  ...
  try {
    switch (req.params.name) {
      ...
      case 'comms_whoami': return text({
        identity: me, sessionName, worker: isWorker, pushEnabled, session: me ? log.session(me) : null,
...
/** Create ~/.heddle/operator.token (0600) once; never overwrite. Prints the path for the operator's .mcp.json. */
function initOperatorToken(): void {
  if (existsSync(OPERATOR_TOKEN_PATH)) {
    process.stdout.write(`operator token already exists: ${OPERATOR_TOKEN_PATH} (not overwritten)\n`);
    return;
  }
  mkdirSync(dirname(OPERATOR_TOKEN_PATH), { recursive: true });
  writeFileSync(OPERATOR_TOKEN_PATH, randomBytes(24).toString('hex') + '\n', { mode: 0o600 });
  process.stdout.write(`operator token written: ${OPERATOR_TOKEN_PATH}\n` +
    `Add to the operator session's .mcp.json env: HEDDLE_COMMS_ROLE=operator, HEDDLE_COMMS_OPERATOR_TOKEN=<contents of that file>\n`);
}

/** The operator binds only through the token credential — never through the agent identity sources. */
function operatorBinding(): boolean {
  if (process.env.HEDDLE_COMMS_ROLE !== 'operator') return false;
  const presented = (process.env.HEDDLE_COMMS_OPERATOR_TOKEN ?? '').trim();
  if (!presented || !existsSync(OPERATOR_TOKEN_PATH)) {
    warn('HEDDLE_COMMS_ROLE=operator but no token presented / no operator token file — refusing to bind operator');
    return false;
  }
  const expected = readFileSync(OPERATOR_TOKEN_PATH, 'utf8').trim();
  const a = Buffer.from(presented), b = Buffer.from(expected);
  if (a.length !== b.length || !timingSafeEqual(a, b)) {
    warn('HEDDLE_COMMS_OPERATOR_TOKEN does not match the operator token file — refusing to bind operator');
    return false;
  }
  return true;
}
...
function resolveCommsIdentity(): string | null {
  if (operatorBinding()) return 'operator';
