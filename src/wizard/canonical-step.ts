  ...
  async run(ctx, io) {
    ...
    if (prior) {
      try {
        ...
        if (!missing.length) return { id: 'canonical', status: 'done', summary: `kept canonical ${canonical}` };
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        return { id: 'canonical', status: 'failed', summary: `could not validate existing canonical at ${configPath}: ${message}` };
      }
    }

    if (envOverride) {
      if (ctx.dryRun) {
        io.report(`would record canonical ${envOverride} (from $HEDDLE_CANONICAL); nothing written`);
        return { id: 'canonical', status: 'skipped', summary: `would record canonical ${envOverride} (from $HEDDLE_CANONICAL)` };
      }
      try {
        const { canonical, missing } = validateCanonical(envOverride);
