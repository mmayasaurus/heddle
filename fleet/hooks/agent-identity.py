...
A session with no short label is not a fleet agent → identity is omitted, but the SCOPE block
(Maya 2026-08-23) is still injected.
...
HEDDLE_FLEET = frozenset("RSTUVWXYZ")  # Y/Z = reserved heddle letters (resume-sessions-hed.sh)
SCOPE_BLOCK = (  # Maya, firsthand 2026-08-23 — absolute; shown at every session start for heddle-fleet and unlabeled sessions
...
def main() -> None:
    label = None  # resolved inside the try; the failure fallback gates the SCOPE block on it
    try:
        try:
        ...
        if label and cache:
            ...
            try:
                # HED-387: pid bridge for the comms channel-server (child of this claude process) —
                # lets a hand-created session bind its comms identity without a relaunch.
                (CACHE_DIR / f"pid-{_claude_ancestor_pid()}.label").write_text(label)
            except Exception:
                pass
            try:
                # HED-387 (on-PR round): baseline the comms nudge mark AT SESSION START so mail
                # arriving between now and the first prompt still nudges (the per-turn seed would
                # treat it as history). Absent-db / any failure: silently skip.
                hwm_file = CACHE_DIR / f"comms-hwm-{label}"
                if not hwm_file.exists() and os.path.exists(COMMS_DB):
                    import sqlite3
        ...
        if not label:
            emit(SCOPE_BLOCK.strip())  # not a fleet session — still show the scope rule (Maya 2026-08-23)
        ...
        # Heddle fleet: point idle/claim lookups at the HED board (lin.sh defaults to SPI — fleet-scope.md).
        lin = ("LIN_TEAM=HED " if label in HEDDLE_FLEET else "") + f".claude/bin/lin.sh --agent {label}"
        ...
        if issues:
            ...
            iss_line = (f"You have no Linear issues claimed right now "
                        f"(`{lin} list` shows ready work).")
        else:
            iss_line = (f"Check your claimed Linear issues: "
                        f"`{lin} mine`.")
        ctx = (
            (SCOPE_BLOCK if label in HEDDLE_FLEET else "") +
        ...
        # Never block a session start over a reminder — but the SCOPE block must survive any failure
        ...
        try:
            if not label or label in HEDDLE_FLEET:
                print(json.dumps({"hookSpecificOutput": {"hookEventName": "SessionStart",
                                                         "additionalContext": SCOPE_BLOCK.strip()}}))
        except Exception:
            pass
        sys.exit(0)
